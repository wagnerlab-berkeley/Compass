...
def parseArgs():
    ...
    parser = argparse.ArgumentParser(
        prog="Compass",
        description="Compass version " + str(__version__) + ". Metabolic Modeling for Single Cells. "
    ...
    parser.add_argument(
        "--data",
        help="Gene expression matrix. Should be a tsv file with one row per gene and one column per sample",
        metavar="FILE",
    )
    ...
    parser.add_argument(
        "--model",
        help="Metabolic Model to Use. Currently supporting: RECON1_mat, RECON2_mat, RECON2.2, Human1, or Mouse1",
        default="RECON2_mat",
    ...
    parser.add_argument(
        "--species",
        help="Species to use to match genes to model. Currently supporting: homo_sapiens or mus_musculus",
    ...
    parser.add_argument(
        "--media",
        help="Which media to simulate. The media file is expected to be a json with upper bounds per reaction, each specific to a given model.",
        default="default-media",
        metavar="MEDIA",
    )

    parser.add_argument("--output-dir", help="Where to store outputs", default=".", metavar="DIR")

    parser.add_argument("--temp-dir", help="Where to store temporary files", default="<output-dir>/_tmp", metavar="DIR")

    parser.add_argument("--torque-queue", help="Submit to a Torque queue", metavar="QUEUE")

    parser.add_argument(
        "--num-processes",
        help="Limit to <N> Processes.  Ignored when submitting job onto a queue",
        type=int,
        metavar="N",
    )
    ...
    parser.add_argument("--single-sample", help=argparse.SUPPRESS, type=int, metavar="N")

    parser.add_argument("--set-license", help="Location of Gurobi license file gurobi.lic", metavar="LICENSE")

    # Arguments to help with schedueler scripts
    parser.add_argument("--sample-range", help=argparse.SUPPRESS, nargs=2)

    parser.add_argument("--reaction-range", help=argparse.SUPPRESS, nargs=2)

    parser.add_argument("--metabolite-range", help=argparse.SUPPRESS, nargs=2)

    parser.add_argument("--generate-cache", help=argparse.SUPPRESS, action="store_true")

    parser.add_argument("--test-mode", help=argparse.SUPPRESS, action="store_true")

    parser.add_argument("--num-threads", help="Number of threads to use per sample", type=int, default=1, metavar="N")
    ...
    parser.add_argument(
        "--turbo-meta-subsystem-models-dir",
    ...
    parser.add_argument("--glucose", type=float, required=False, help=argparse.SUPPRESS)

    parser.add_argument("--oxygen", type=float, required=False, help=argparse.SUPPRESS)

    parser.add_argument("--close-oxygen", action="store_true", help="Set lower bound of oxygen exchange reaction to 0")
    ...
    parser.add_argument("--symmetric-kernel", action="store_true", help="Use symmetric TSNE kernel (slower)")

    parser.add_argument(
        "--input-weights", help="File with input sample to sample weights", required=False, metavar="FILE"
    )

    parser.add_argument(
        "--penalty-diffusion",
        help="Mode to use to share reaction penalty values between single cells",
        choices=["gaussian", "knn"],
        metavar="MODE",
        default="knn",
    )

    parser.add_argument("--no-reactions", action="store_true", help="Skip computing scores for reactions")

    parser.add_argument(
        "--calc-metabolites", action="store_true", help="Compute scores for metabolite uptake/secretion"
    )

    parser.add_argument("--precache", action="store_true", help="Preprocesses the model to find  maximum fluxes")

    parser.add_argument(
        "--input-knn",
        help="File with a precomputed knn graph for the samples. "
    ...
    parser.add_argument(
        "--output-knn",
        help="File to save kNN of data to. "
    ...
    parser.add_argument(
        "--latent-space",
        help="File with latent space reprsentation of samples for knn clustering or microclustering. "
    ...
    parser.add_argument(
        "--only-penalties",
        help="Flag for Compass to only compute the reaction penalties for the dataset.",
        action="store_true",
        default=None,
    )
    ...
    parser.add_argument(
        "--example-inputs",
        help="Flag for Compass to list the directory where example inputs can be found.",
        action="store_true",
        default=None,
    )

    parser.add_argument(
        "--microcluster-size", type=int, metavar="C", default=None, help="Target number of cells per microcluster"
    )
    ...
    parser.add_argument("--anndata-output", help="Enables output as .h5ad format", action="store_true")

    # Hidden argument for any potential anndata obs or uns
    parser.add_argument("--anndata-obs", help=argparse.SUPPRESS, default=None)
    parser.add_argument("--anndata-uns", help=argparse.SUPPRESS, default=None)

    # Hidden argument which tracks more detailed information on runtimes
    parser.add_argument("--detailed-perf", action="store_true", help=argparse.SUPPRESS)

    # Hidden argument for testing purposes.
    parser.add_argument("--penalties-file", help=argparse.SUPPRESS, default="")
    ...
    parser.add_argument("--penalties-dir", help=argparse.SUPPRESS, default="")
    ...
    parser.add_argument("--lpmethod", help=argparse.SUPPRESS, default=None, type=int)
    ...
    parser.add_argument("--advance", help=argparse.SUPPRESS, default=2, type=int)

    # Hidden argument to save argmaxes in the temp directory
    parser.add_argument("--save-argmaxes", action="store_true", help=argparse.SUPPRESS)
    ...
    if not args["data"] and not args["data_mtx"]:
        if (
            not args["precache"]
            and not args["list_genes"]
            and not args["example_inputs"]
            and not args["list_reactions"]
            and not args["set_license"]
        ):
            parser.error(
                "Nothing selected to do. Add arguments --data, --data-mtx, --precache, --list-genes, --list-reactions, --example-inputs, or --set-license for Compass to do something."
            )
    else:
        if args["data_mtx"]:
    ...
    if args["lambda"] < 0 or args["lambda"] > 1:
        parser.error("'lambda' parameter cannot be less than 0 or greater than 1")

    if args["turbo"] < 0 or args["turbo"] > 1:
        parser.error("'turbo' parameter cannot be less than 0 or greater than 1")

    if args["generate_cache"] and (args["no_reactions"] or not args["calc_metabolites"]):
        parser.error(
...
def entry():
    ...
    if (not args["set_license"]) and (not os.path.exists(os.path.join(globals.LICENSE_DIR, "gurobi.lic"))):
        print(
            "A Gurobi license is required to run Compass. Run Compass again with '--set-license <PATH_TO_LICENSE>' to save license."
        )
        return
    ...
    if args["data"]:
        ...
        if not os.path.isdir(args["temp_dir"]) and args["temp_dir"] != "/dev/null":
            ...
            if len(diffs) > 0:
                table = pd.DataFrame(
                    {"temp_dir": {x: temp_args[x] for x in diffs}, "current": {x: args[x] for x in diffs}}
                )
                print(
                    "Warning: The arguments used in the temporary directory (",
                    args["temp_dir"],
                ...
                print(table)
                print(
                    "Enter 'y' or 'yes' if you want to use cached results.\n",
    ...
    try:
        commit = sp.check_output(
            ["git", "--git-dir", globals.GIT_DIR, "rev-parse", "--short", "HEAD"], stderr=open(os.devnull, "w")
        )
...
def compass_work(args, logger, start_time):
    ...
    if args["turbo"] < 1.0:
        ...
        if args["select_meta_subsystems"]:
            ...
            for meta_subsystem_model in model_names:
                ...
                if os.path.exists(success_token):
                    ...
                    penalties = eval_reaction_penalties(
                        args["data"],
                        meta_subsystem_model,
                        args["media"],
                        args["species"],
                        args,
                        metabolic_model_dir=meta_subsystem_models_dir,
        ...
        # If we are using regular Turbo-Compass, then compute penalties beforehand to reuse for future iterations
            ...
            if os.path.exists(success_token):
                ...
                logger.info("Evaluating Reaction Penalties...")
                penalties = eval_reaction_penalties(args["data"], args["model"], args["media"], args["species"], args)
    ...
    global_state.init_selected_reactions_for_each_cell(args.get("selected_reactions_for_each_cell", None))
    ...
    if args["microcluster_size"] and args["data"]:
        ...
        if os.path.exists(microcluster_success_token):
            ...
            pools = microcluster(
                data,
                cellsPerPartition=args["microcluster_size"],
                latentSpace=args["latent_space"],
                inputKnn=args["input_knn"],
                inputKnnDistances=args["input_knn_distances"],
    ...
    if args["single_sample"] is not None:
        if args["select_meta_subsystems"]:
            ...
            if args["turbo_meta_subsystem_model"] is not None:
                ...
                args["select_reactions"] = os.path.join(
                    meta_subsystem_models_dir, meta_subsystem_model, f"{meta_subsystem_model}_rxns.txt"
                )
                ...
                if os.path.exists(success_token):
                    ...
                    logger.info(
                        "Penalties file is corrupted. Please start a new Turbo-Module run to re-compute reaction penalties."
                ...
                singleSampleCompass(
                    ...
                    model=meta_subsystem_model,
                    media=args["media"],
                    directory=os.path.join(args["temp_dir"], meta_subsystem_model),
            ...
            # Normal Module-Compass
                ...
                for meta_subsystem_model in model_names:
                    ...
                    if os.path.exists(success_token):
                        ...
                        for argument in arguments:
                            pool.apply_async(
                                save_single_sample_penalties, args=argument, callback=lambda _: pbar.update()
                            )
                    ...
                    singleSampleCompass(
                        ...
                        model=meta_subsystem_model,
                        media=args["media"],
                        directory=meta_subsystem_model_temp_dir,
                        sample_name=sample_name,
                        sample_index=args["single_sample"],
                        args=args,
                        metabolic_model_dir=meta_subsystem_models_dir,
            ...
            if args["turbo_temp_dir"] is not None:
                ...
                if os.path.exists(success_token):
                    ...
                    # NOTE: if run correctly this branch should not be accessed
                    logger.info(
                        "Penalties file is corrupted. Please start a new Turbo-Compass run to re-compute reaction penalties."
                        "You may need to all files in the output directory."
                    )
                    return
            ...
            # Normal Compass
                ...
                if os.path.exists(success_token):
                    ...
                    logger.info("Evaluating Reaction Penalties...")
                    penalties = eval_reaction_penalties(
                        args["data"], args["model"], args["media"], args["species"], args
                    )
    ...
    if args["collect"]:
        collectCompassResults(args["data"], args["temp_dir"], args["output_dir"], args)
        end_time = datetime.datetime.now()
        logger.debug("\nElapsed Time: {}\n".format(end_time - start_time))
        return
    ...
    if args["select_meta_subsystems"]:
        ...
        if args["turbo_meta_subsystem_model"] is not None:
            ...
            if args["torque_queue"] is not None:
                logger.info("Submitting COMPASS job to Torque queue - {}".format(args["torque_queue"]))
                submitCompassTorque(
                    args, temp_dir=args["temp_dir"], output_dir=args["output_dir"], queue=args["torque_queue"]
                )
                return
            else:
                runCompassParallel(
...
def runCompassParallel(
    ...
    logger.info("Processing {} samples using {} processes".format(n_samples, args["num_processes"]))

    logger.info("Progress bar will update once the first sample is finished")
    ...
    results = []
    for argument in arguments:
        res = pool.apply_async(partial_map_fun, args=argument, callback=lambda _: pbar.update())
        results.append(res)
    ...
    collectCompassResults(
        args["data"], temp_dir, output_dir, args, model_name=model_name, metabolic_model_dir=metabolic_model_dir
    )

    logger.info("COMPASS Completed Successfully")


def _parallel_map_fun(
    sample_name,
    i,
    args,
    model_name=None,
...
def collectCompassResults(data, temp_dir, out_dir, args, model_name=None, metabolic_model_dir=MODEL_DIR):
    ...
    for i in range(n_samples):
        ...
        try:
            reactions = pd.read_csv(os.path.join(sample_dir, "reactions.txt"), sep="\t", index_col=0)
        ...
        try:
            secretions = pd.read_csv(os.path.join(sample_dir, "secretions.txt"), sep="\t", index_col=0)
        ...
        try:
            uptake = pd.read_csv(os.path.join(sample_dir, "uptake.txt"), sep="\t", index_col=0)
...
def load_config(args):
    ...
    if "config_file" not in args or args["config_file"] is None:
        return
...
def precacheCompass(args, model_name=None, metabolic_model_dir=MODEL_DIR, preprocess_cache_dir=PREPROCESS_CACHE_DIR):
    ...
    if n_processes > 1:
        ...
        reaction_chunks = [
            (i * reaction_chunk_size, min(n_reactions, (i + 1) * reaction_chunk_size)) for i in range(n_processes)
        ]

        combined_cache = {}
        partial_map_fun = partial(
            _parallel_map_precache_reactions, args=args, model_name=model_name, metabolic_model_dir=metabolic_model_dir
        )
        pool = utils.create_process_pool(args)
        ...
        partial_map_fun = partial(
            _parallel_map_precache_metabs, args=args, model_name=model_name, metabolic_model_dir=metabolic_model_dir
        )
        pool = utils.create_process_pool(args)
