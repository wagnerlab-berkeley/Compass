...
from .metrics import (
    ...
    compute_pearson_r2,
    compute_mse,
    compute_spearman_r2s,
    compute_variance_explained,
    compute_variance_explained_fast,
)
...
def plot_prediction(
    ...
    """
    indices_to_use = (
        np.where(np.isnan(y_observed)) if exclude_observed_for_plotting else np.where(~np.isnan(y_true))
    )  # Hacky way to get all entries...
    y_true_plotting, y_completion_plotting = y_true[indices_to_use], y_completion[indices_to_use]
...
def plot_predictions(
    ...
    np.random.seed(seed)
    for i, r in enumerate(np.random.choice(range(X.shape[1]), size=k * k, replace=False)):
        a[i // k][i % k].axis("off")
        indices_to_use = (
            np.where(np.isnan(X_observed[:, r])) if exclude_observed else np.where(~np.isnan(X[:, r]))
        )  # Hacky way to get all entries...
        ...
        if metric == "r2":
            r2 = compute_r2(X[:, r], X_observed[:, r], X_completion[:, r], exclude_observed=exclude_observed_for_metric)
            a[i // k][i % k].set_title("r2 = %.2f" % r2)
        elif metric == "spearman_r2":
            spearman_r2 = compute_spearman_r2(
                X[:, r], X_observed[:, r], X_completion[:, r], exclude_observed=exclude_observed_for_metric
            )
            a[i // k][i % k].set_title("s_r2 = %.2f" % spearman_r2)
        elif metric == "pearson_r2":
            pearson_r2 = compute_pearson_r2(
                X[:, r], X_observed[:, r], X_completion[:, r], exclude_observed=exclude_observed_for_metric
            )
...
def compare_hypothesis_test_to_ground_truth(
    ...
    hypothesis_tests = pd.merge(
        hypothesis_test_true, hypothesis_test_pred, left_index=True, right_index=True
    )  # show_HT_in_one_direction operated on this object
    # Keep populating the hypothesis_tests object, then sanity check all data.
    hypothesis_tests["p_value_ratio"] = np.log10(hypothesis_tests["p_val_x"] / hypothesis_tests["p_val_y"])
    hypothesis_tests["p_value_ratio_abs"] = np.abs(hypothesis_tests["p_value_ratio"])
    hypothesis_tests["-|score|_diff"] = hypothesis_tests["-|score|_x"] - hypothesis_tests["-|score|_y"]
    hypothesis_tests["-|score|_diff_abs"] = np.abs(hypothesis_tests["-|score|_diff"])

    def show_HT_in_one_direction(
        hypothesis_tests: pd.DataFrame, direction: Optional[int], rank_col: str, nfeatures: int
    ):
        r"""
        ...
        features_to_use = (
            (direction * hypothesis_tests["cohen_d_x"]) > 0
        ...
        print(
            f"%.2f pct p-values deteriorate (WARNING: Ties in ranks broken arbitrarily!)"
    ...
    if show_largest_p_val_changes:
        least_deteriorated = list(
            hypothesis_tests.sort_values(["-|score|_diff"], ascending=False).index[: (k * k + 1) // 2]
        )
        most_deteriorated = list(
            hypothesis_tests.sort_values(["-|score|_diff"], ascending=False).index[-(k * k - (k * k + 1) // 2) :]
        )
        generator = enumerate(least_deteriorated + most_deteriorated)
        print(
            f"=> Showing {len(least_deteriorated)} & {len(most_deteriorated)} most extreme "
    ...
    if not (group_0 | group_1).all():
        raise ValueError(
            f"Not all observations belong to group 0 or 1. "
    ...
    for i, feature in generator:
        ...
        a[2 * (i // k)][i % k].set_title(
            "%s\n%.2f / %.2f"
            % (feature, hypothesis_tests.loc[feature, "-|score|_x"], hypothesis_tests.loc[feature, "-|score|_y"])
        )
...
def analyze_imputed_hypothesis_test(
    ...
    feature_filter_func: Callable[[str], bool] = lambda x: True,
    cv_spearman_r2_df: Optional[pd.DataFrame] = None,
    use_observed_in_completion: bool = False,
) -> None:
    r"""
    ...
    print(
        f"Subsetting only observations from the two groups involved in the hypothesis test: "
    ...
    print(
        "Variance explained fast (pseudo_std=1e-1) = %.2f"
    ...
    def hypothesis_test_func(X: np.array, subset_obs: Optional[int] = None):
        if subset_obs:
            print(f"WARNING: Performing hypothesis test on a subset of {min(X.shape[1], subset_obs)} observations.")
            np.random.seed(seed)
            random_obs = np.random.choice(obs_metadata.index, min(X.shape[1], subset_obs), replace=False)
        else:
    ...
    def compare_hypothesis_test_to_ground_truth_func(
        X_df_pred, hypothesis_test_pred, spearman_r2s_df, spearman_r2s_mean
    ):
    ...
    print("*" * 50)
    compare_hypothesis_test_to_ground_truth_func(X_df_pred, hypothesis_test_pred, spearman_r2s_df, spearman_r2s_mean)
    print("*" * 51)
    print("*" * 15 + " OBSERVATION METRICS " + "*" * 15)
    print("*" * 51)
    compare_hypothesis_test_to_ground_truth_func(
        X_df_observed, hypothesis_test_observed, spearman_r2s_df * 0 + 1.0, 1.0
    )
