...
def microcluster(
    ...
    if inputKnn is not None:
        ...
        if latentSpace is not None:
            ...
            log_expression_filtered = applyFilters(log_expression, filterInput, filterThreshold, filterNumMad)

            if len(log_expression_filtered) == 0:
                raise Exception("0 genes were selected with current threshold, set a lower one and rerun")

            model = PCA(
                n_components=min(log_expression_filtered.shape[0], log_expression_filtered.shape[1], 20),
            ...
            pca_expression = pd.DataFrame(pca_expression, columns=exprData.columns)
            res = pca_expression
    ...
    cl = leidenalg.find_partition(
        igraph.Graph.Weighted_Adjacency(adj), leidenalg.ModularityVertexPartition, seed=LEIDEN_SEED
    )
    ...
    while currPart < ((1 - EPSILON) * NUM_PARTITIONS):
        ...
        for i in range(len(clusters)):
            ...
            if len(currCl) > cellsPerPartition:
                nCl = KMeans(n_clusters=round(subData.shape[0] / cellsPerPartition), random_state=KMEANS_SEED).fit(
                    subData
                )
            else:
...
def pool_matrix_cols(data, pools):
    ...
    return (
        data.append(groups)
        .T.groupby("compass_microcluster")
        .mean()
        .T.rename(mapper=lambda x: "cluster_" + str(int(x)), axis=1)
    )
