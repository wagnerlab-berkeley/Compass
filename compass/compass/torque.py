...
def submitCompassTorque(args, temp_dir, output_dir, queue):
    ...
    exclude_args = {"temp_dir", "output_dir", "torque_queue", "num_processes"}
    newArgs = {}
    ...
    # Save the arguments to the config file
    exclude_args = {"output_dir", "torque_queue", "num_processes", "collect"}
    newArgs = {}
    for arg, val in args.items():
        if val is None:
            continue
    ...
    command_args = [
        ...
        "-V",
        "-W",
        "depend=afteranyarray:" + array_job_id,
        "-d",
        output_dir,
        "-F",
        " ".join(script_args),
    ...
    logger.info("Compass submitted as array job {} and collect job {}".format(array_job_id, collect_job_id))
