...
import pandas as pd
import numpy as np
from compass.models import MetabolicModel, Reaction, Species, Association, Gene
...
for x in tqdm(range(1000)):
    ...
    expected_max["R3_pos"] = min(expected_max["R1_pos"], expected_max["R2_pos"] / 2)
    ...
    expected_max["M3_UPTAKE"] = min(
        ...
        + min(model.reactions["E1_neg"].upper_bound, model.reactions["R1_neg"].upper_bound),
        EXCHANGE_LIMIT,
    )
    ...
    expected_max["M4_UPTAKE"] = min(
        ...
        + min(model.reactions["E2_neg"].upper_bound, model.reactions["R2_neg"].upper_bound),
        EXCHANGE_LIMIT,
    )
    ...
    expected_max["M3_SECRETION"] = min(model.reactions["R1_pos"].upper_bound, model.reactions["E1_pos"].upper_bound)

    expected_max["M4_SECRETION"] = min(model.reactions["R2_pos"].upper_bound, model.reactions["E2_pos"].upper_bound)
